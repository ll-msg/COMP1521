...
void access_page(int virtual_page, int access_time, int n_physical_pages, struct ipt_entry *ipt) {
    ...
    //    and there is free physical pag;
    //
    for (int i = 0; i < n_physical_pages; i++) {
    ...
    //
    int smallest = ipt[0].last_access_time;
    int index = 0;
    for (int i = 0; i < n_physical_pages; i++) {
        if (ipt[i].last_access_time < smallest && ipt[i].last_access_time != -1) {
            smallest = ipt[i].last_access_time;
            index = i;
        }
    }
    int old_page = ipt[index].virtual_page;
    ipt[index].virtual_page = virtual_page;
    ipt[index].last_access_time = access_time;
    printf("Time %d: virtual page %d  - virtual page %d evicted - loaded to physical page %d\n", 
                                    access_time, virtual_page, old_page, index);
    return;
}
